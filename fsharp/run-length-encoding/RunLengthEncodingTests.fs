...
[<Fact>]
let ``Encode empty string`` () = encode "" |> should equal ""

[<Fact>]
let ``Encode single characters only are encoded without count`` () = encode "XYZ" |> should equal "XYZ"
...
[<Fact>]
let ``Decode empty string`` () = decode "" |> should equal ""

[<Fact>]
let ``Decode single characters only`` () = decode "XYZ" |> should equal "XYZ"
